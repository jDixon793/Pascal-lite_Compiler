...
vector<string> keywords = {"program","const","var","integer","boolean","begin","end","true","false","not","mod","div","and","or","read","write","if","then","else","repeat","while","do","until"};
...
int main(int argc, char **argv)
{
   ...
   catch (int i)
   {
      ...
      list<<"\nError: Line "<<lineNum-1<<": ";
       ...
       {
         case 1:
            list<< "keyword \"program\" expected, found "<<token<<endl;
            break;
         case 2:
         ...
         case 3:
            ...
            break;
         case 4:
            list<<"'_' must be followed by an ALPHANUM, found "<<charac<<endl;
            break;
         case 5:
            list<<"'_' cannot end token"<<endl;
            break;
         case 6:
            list<<"illegal symbol "<<token<<" found"<<endl;
            break;
         case 7:
            list<<"program name expected, found"<<token<<endl;
            break;
         case 8:
            list<<"semicolon expected, found "<<charac<<endl;
            break;
         case 9:
            list<<"keyword \"const\" expected, found"<<token<<endl;
            break;
         case 10:
         ...
         case 11:
            ...
            break;
         case 12:
            list<<"illegal use of keyword "<<token<<endl;
            break;
         case 13:
            list<<"non-keyword identifier expected"<<endl;
            break;
         case 14:
            list<<"\"=\" expected, found "<<token<<endl;
            break;
         case 15:
            list<<"token to right of \"=\" illegal"<<endl;
            break;
         case 16:
            list<<"reference to undefined constant, "<<token<<endl;
            break;
         case 17:
         ...
         case 18:
            ...
            break;
         case 19:
            list<<"non-keyword identifier, \"begin\", or \"var\" expected, found "<<token<<endl;
            break;
         case 20:
            list<<"keyword \"var\" expected, found "<<token<<endl;
            break;
         case 21:
            list<<"non-keyword identifier must follow \"var\""<<endl;
            break;
         case 22:
            list<<"\":\" or \",\" expected, found "<<token<<endl;
            break;
         case 23:
            list<<"illegal type follows \":\""<<endl;
            break;
         case 24:
            list<<"non-keyword identifier or \"begin\" expected, found "<<token<<endl;
            break;
         case 25:
            list<<"keyword \"begin\" expected, found "<<token<<endl;
            break;
         case 26:
            list<<"non-keyword identifier, \"read\", \"write\", \"if\", \"while\", \"repeat\", \";\", or  \"begin\" expected, found "<<token<<endl;
            break;
         case 27:
            list<<"period expected after last keyword \"end\""<<endl;
            break;
         case 28:
		 ...
		 case 33:
			...
			break;
		 case 34:
			list<<"\"(\", BOOLEAN, or NON_KEY_ID expected, found "<<token<<endl;
			break;
		 case 35:
			list<<"\"(\", INTEGER, or NON_KEY_ID expected, found "<<token<<endl;
			break;
		 case 36:
			list<<"relational operator expected, found "<<token<<endl;
			break;
		 case 37:
			list<<"add level operator expected, found "<<token<<endl;
			break;
		 case 38:
			list<<"multiplication level operator expected, found "<<token<<endl;
			break;
         case 39:
         ...
         case 41:
            ...
            break;
         case 42:
            list<<"cannot read in CONSTANT "<<errorSource<<endl;
            break;
		 case 43:
		 ...
		 case 46:
			...
			break;
		 case 47:
			list<<"illegal type; (" << errorSource << ") operator requires operands of matching type"<<endl;
			break;
         case 48:
            list<<"illegal type; operator (" << errorSource << ") requires boolean operands"<<endl;
            break;
         case 49:
            list<<"illegal type; operator (" << errorSource << ") requires integer operands"<<endl;
            break;
		case 50:
            list<<"illegal type; operator (" << errorSource << ") requires a boolean operand"<<endl;
            break;
         case 51:
            list<<"illegal type; operator (" << errorSource << ") requires an integer operand"<<endl;
            break;
         case 52:
            list<<"semicolon expected after keyword \"end\""<<endl;
            break;
          case 53:
            list<<"keyword \"then\" expected after if's expression, found "<<token<<endl;
            break;  
          case 54:
            list<<"keyword \"do\" expected after while's expression, found "<<token<<endl;
            break;
         case 55:
            list<<"keyword \"until\" expected after repeats's expressions, found "<<token<<endl;
            break;
       }
   }
...
void ConstStmts()
{
   ...
   {
		NextToken();
		//if it is a defined boolean symbol
		if(isNonKeyID(token) && isDefined(token) && getEntry(token).dataType==BOOLEAN)
		{
			if(getEntry(token).value == "1")
		  {
			 y = "false";
		  }
		  else
...
void BeginEndStmt() //token should be "begin"
{
   ...
   unmatchedBegin++;
   if(isNonKeyID(NextToken())||token == "read"||token == "write"||token == "begin"||token == "if"||token == "while"||token == "repeat"||token == ";")
   {
       ExecStmts();
   ...
   //if we have matched all begins the NextToken should be .
   NextToken();
   if(unmatchedBegin == 0 && token != ".")
   {
      throw 27;
   }
   //if we have not matched all begins the NextToken should be ;
   else if (unmatchedBegin > 0 && token  != ";")
   {
       throw 52;
   }
   
   //if we have matched all begins and the next token is . then emit end code
   if(unmatchedBegin == 0 && token == ".")
   {
        Code("end","","");
       NextToken();
...
void ExecStmts()
{
    //need to change this error to reflect all of the new selection set
    ...
    //ExecStmt is in charge of loading up the next Token
    if(isNonKeyID(token)||token == "read"||token == "write"||token == "begin" ||token == ";"||token == "if"||token == "while"||token == "repeat")
    {
...
void ExecStmt()
{
    ...
    else if(token == "begin")//keyword write
    {
        BeginEndStmt();
        NextToken();
    }
...
void Terms()
{
	if (token == "-" || token == "+" || token == "or")
	{
		...
		Terms();
				...
				||token == ")" ||token == ";"|| token =="do"||token=="then")
	{
		return;
	}
	else
...
void Factors()
{
	if (token == "*" || token == "div" || token == "mod" ||token == "and")
	{
		...
		Factors();
			...
			||token == ")" ||token == ";" ||token == "-" ||token == "+" ||token == "or"|| token =="do"||token=="then")
	{
		return;
	}
	else
...
{		
    ...
    else if(operation == "post_while")
    {
        EmitPostWhileCode(op1,op2);
    }
...
{
    string tempLabel = "L" + to_string(labelCount);
    string location,opCode,address,signNumber,comments;
    location=opCode=address=signNumber=comments ="";
	...
	opCode = "AZJ";
	address = tempLabel;
	comments = "if "+getExternalName(op1)+" is false then goto "+tempLabel;
	obj<<setw(4)<<left<<location<<right<<setw(5)<<opCode<<" "<<setw(4)<<left<<address<<right<<setw(2)<<signNumber<<setw(3)<<""<<comments<<endl;
	signNumber = "";
   ...
   if(curA != op1)
	{
        opCode = "LDA";
        address = op1;
        comments = "load " + op1 + " into the A register";
        obj<<setw(4)<<left<<location<<right<<setw(5)<<opCode<<" "<<setw(4)<<left<<address<<right<<setw(2)<<signNumber<<setw(3)<<""<<comments<<endl;
        signNumber = "";
    }
...
bool isLabel(string name)    //checks to see if the name is a temp
{
    return name[0]=='L'&&isDigit(name[1]);
}
